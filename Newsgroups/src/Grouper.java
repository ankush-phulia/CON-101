...
public class Grouper {
	...
	public static void builder_excl(int k) throws FileNotFoundException{
		...
		while (s.hasNextLine()){
			...
			double d1=base.prob(words, tmp, voc)+Math.log(artcnts.get(base))-Math.log(articles);
			double d2=guns.prob(words, tmp, voc)+Math.log(artcnts.get(guns))-Math.log(articles);
			double d3=rmisc.prob(words, tmp, voc)+Math.log(artcnts.get(rmisc))-Math.log(articles);
			double d4=pmisc.prob(words, tmp, voc)+Math.log(artcnts.get(pmisc))-Math.log(articles);
			double d5=hock.prob(words, tmp, voc)+Math.log(artcnts.get(hock))-Math.log(articles);
			double d6=mideast.prob(words, tmp, voc)+Math.log(artcnts.get(mideast))-Math.log(articles);
			double d7=autos.prob(words, tmp, voc)+Math.log(artcnts.get(autos))-Math.log(articles);
			double d8=moto.prob(words, tmp, voc)+Math.log(artcnts.get(moto))-Math.log(articles);
			double[] drr={d1,d2,d3,d4,d5,d6,d7,d8};
			/*double d0=Double.max(Double.max(Double.max(d1, d2),Double.max(d4, d3)),Double.max(Double.max(d5, d6),Double.max(d7, d8)));*/
			double d0=drr[0];
			int ind=0;
			for (int q=1;q<8;q++){
				if (drr[q]>d0){
			...
			String[] categ={"rec.sport.baseball","talk.politics.guns","talk.religion.misc","talk.politics.misc","rec.sport.hockey","talk.politics.mideast","rec.autos","rec.motorcycles"};
